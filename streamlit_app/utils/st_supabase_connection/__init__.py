...
from streamlit_app.core.supabase.supabase_auth import Client, create_client
...
class SupabaseConnection(BaseConnection[Client]):
    ...
    def download(
        ...
        def _download(_self, bucket_id, source_path):
            ...
            with open(file_name, "wb+") as f:
                response = _self.client.storage.from_(bucket_id).download(
                    source_path)
                f.write(response)
    ...
    def update_bucket(
        ...
        response = self.client.storage._request("PUT",
                                                f"/bucket/{bucket_id}",
                                                json=json)
        return response.json()

    def move(self, bucket_id: str, from_path: str,
             to_path: str) -> "dict[str, str]":
    ...
    def list_objects(
        ...
        limit: int = 100,
        offset: int = 0,
        sortby: Optional[Literal["name", "updated_at", "created_at",
                                 "last_accessed_at"]] = "name",
        ...
        @cache_data(ttl=ttl)
        def _list_objects(_self, bucket_id, path, limit, offset, sortby,
                          order):
        ...
        return _list_objects(self, bucket_id, path, limit, offset, sortby,
                             order)
    ...
    def create_signed_urls(
        ...
        for item in data:
            if item["signedURL"]:
                item[
                    "signedURL"] = f"{self.client.storage._client.base_url}{item['signedURL'].lstrip('/')}"
    ...
    def get_public_url(
        ...
        def _get_public_url(_self, bucket_id, filepath):
            return _self.client.storage.from_(bucket_id).get_public_url(
                filepath)

        return _get_public_url(self, bucket_id, filepath)

    def create_signed_upload_url(self, bucket_id: str,
    ...
    def upload_to_signed_url(
        ...
        if source == "local":
            ...
            with open(file, "rb") as f_obj:
                _file = {
                    "file": (filename, f_obj, mimetypes.guess_type(file)[0])
                }
                response = self.client.storage.from_(bucket_id)._request(
                    "PUT",
                    final_url,
...
def execute_query(
    query: Union[SyncSelectRequestBuilder, SyncQueryRequestBuilder,
                 SyncFilterRequestBuilder],
    ttl: Optional[Union[float, timedelta, str]] = None,
) -> APIResponse:
    """Execute the query.
