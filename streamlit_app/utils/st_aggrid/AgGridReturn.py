...
class AgGridReturn(Mapping):
    ...
    def __process_vanilla_df_response(self, nodes,
                                      __try_to_convert_back_to_original_types):
        data = pd.DataFrame([
            n.get("data", {}) for n in nodes
            if not n.get("group", False) == True
        ])
        ...
        if __try_to_convert_back_to_original_types:
            ...
            if numeric_columns:
                data.loc[:,
                         numeric_columns] = data.loc[:, numeric_columns].apply(
            ...
            timedelta_columns = [
                k for k, v in original_types.items() if v == "m"
            ]
            if timedelta_columns:
                ...
                data.loc[:,
                         timedelta_columns] = data.loc[:,
                                                       timedelta_columns].apply(
                                                           cast_to_timedelta)

        return data

    def __process_grouped_response(self, nodes,
                                   __try_to_convert_back_to_original_types,
                                   __data_return_mode):
        ...
        data = [{
            **i.get("data"),
            **{
                "parent": travel_parent(i)
            }
        } for i in nodes if i.get("group", False) == False]
    ...
    def __get_data(self, onlySelected):
        ...
        if self.__component_value_set:
            ...
            if onlySelected:
                nodes = list(
                    filter(lambda n: n.get('isSelected', False) == True,
                           nodes))
            ...
            data = self.__process_vanilla_df_response(
                nodes, self.__try_to_convert_back_to_original_types
                and onlySelected)

            reindex_ids_map = {
                DataReturnMode.FILTERED:
                self.rows_id_after_filter,
                DataReturnMode.FILTERED_AND_SORTED:
                self.rows_id_after_sort_and_filter
            }
    ...
    def __get_dataGroups(self, onlySelected):
        if self.__component_value_set:
            ...
            if onlySelected:
                ...
                nodes = list(
                    filter(lambda n: n.get('isSelected', True) == True, nodes))
    ...
    def selected_rows(self):
        ...
        selected_items = pd.DataFrame(
            self.grid_response.get("selectedItems", {}))
    ...
    def __iter__(self):
        attrs = (x for x in inspect.getmembers(self)
                 if not x[0].startswith('_'))
        return attrs.__iter__()

    def __len__(self):
        attrs = [
            x for x in inspect.getmembers(self) if not x[0].startswith('_')
        ]
        return attrs.__len__()

    def keys(self):
        return [
            x[0] for x in inspect.getmembers(self) if not x[0].startswith('_')
        ]

    def values(self):
        return [
            x[1] for x in inspect.getmembers(self) if not x[0].startswith('_')
        ]
