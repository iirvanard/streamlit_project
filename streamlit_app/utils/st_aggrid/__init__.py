...
from .shared import GridUpdateMode, DataReturnMode, JsCode, walk_gridOptions, ColumnsAutoSizeMode, AgGridTheme, ExcelExportMode
...
def __parse_row_data(data):
    ...
    if isinstance(data, pd.DataFrame):
        ...
        frame_dtypes = dict(
            zip(data_parameter.columns,
                (t.kind for t in data_parameter.dtypes)))
        del data_parameter['__pandas_index']
        return json.loads(row_data), frame_dtypes

    elif isinstance(data_parameter, str):
        is_path = data_parameter.endswith('.json') and os.path.exists(
            data_parameter)

        if is_path:
            row_data = json.dumps(
                json.load(open(os.path.abspath(data_parameter))))
        else:
...
def __parse_grid_options(gridOptions_parameter, data,
                         default_column_parameters, unsafe_allow_jscode):
    ...
    if (gridOptions_parameter == None) and not (data is None):
        ...
        if is_path:
            gridOptions = json.load(
                open(os.path.abspath(gridOptions_parameter)))
        else:
    ...
    if unsafe_allow_jscode:
        ...
        walk_gridOptions(gridOptions, lambda v: v.js_code
                         if isinstance(v, JsCode) else v)
...
def AgGrid(
    ...
    **default_column_parameters:
    ...
    gridOptions = __parse_grid_options(gridOptions, data,
                                       default_column_parameters,
                                       allow_unsafe_jscode)
    ...
    try:
        ...
        args = list(ex.args)
        args[
            0] += ". If you're using custom JsCode objects on gridOptions, ensure that allow_unsafe_jscode is True."
